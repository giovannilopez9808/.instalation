...
def fetch_suggestions(search_string):
    if SEARCH_ENGINE == 'google':
        ...
        url = CONFIG['SUGGESTION_URL'][SEARCH_ENGINE] + \
            urllib.parse.urlencode(r)
        headers = {
            'sec-fetch-mode': 'cors',
            'dnt': '1',
        ...
        reply_data = gzip.decompress(
            urllib.request.urlopen(req).read()).split(b'\n')[1]
...
def main():
    ...
    if search_string.endswith('!'):
        ...
        url = CONFIG['SEARCH_URL'][SEARCH_ENGINE] + \
            urllib.parse.quote_plus(search_string)
        sp.Popen(CONFIG['BROWSER_PATH'][BROWSER] + [url],
                 stdout=sp.DEVNULL, stderr=sp.DEVNULL, shell=False)
...
def validate_config(c):
    ...
    for k in ('SEARCH_ENGINE', 'BROWSER'):
        if type(c[k]) != str:
            print('Configuration Error: The value of %s must be a string' %
                  k, file=sys.stderr)
    ...
    for x in c['TERMINAL']:
        if type(x) != str:
            print(
                'Configuration Error: The value of TERMINAL must be a list of strings', file=sys.stderr)
            sys.exit(1)
    ...
    try:
        ...
        if os.path.exists(fname):
            try:
                ...
                print('Configuration file %s is not a valid JSON' %
